...
- [ ] M04: **First Private Commit:** The initial project state will be committed after this documentation pass.
- [x] M05: **Framework/Ruleset Established:** The core memory bank files and AI rules are being populated now.

**Phase 2: Core Development**
- [ ] M06: **Implement Script Foundation:** Create `privatize-folders.sh` with argument parsing, logging, locking, and usage functions.
- [ ] M07: **Implement Core File Logic:** Add the folder moving/creation and relative symlinking logic.
- [ ] M08: **Implement `.gitignore` Management:** Develop the logic to intelligently update the `.gitignore` file (uncommenting or adding new entries).
- [ ] M09: **Develop Utility Functions:** Create the shared `logging_utils.sh` and `messaging_utils.sh` files, adapting from the reference script.
- [ ] M10: **Finalize Configuration Handling:** Ensure the script correctly reads and processes the folder list from the `.config` file.
...
- [ ] M13: **Document Core Systems:** Finalize all in-script comments and ensure the `README.md` is complete and accurate.
- [ ] M14: **Perform Code Refactor Pass:** Review the final script for clarity, efficiency, and adherence to shell scripting best practices.
- [ ] M15: **(N/A)**
...
*   [ ] Q1: Implement the idempotent `.gitignore` logic to handle both uncommenting existing lines and cleanly adding a new managed block if one doesn't exist.
*   [ ] Q2: Ensure all file and folder path resolutions are robust and work correctly regardless of the script's execution location.
